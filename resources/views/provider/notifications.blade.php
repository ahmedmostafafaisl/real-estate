<x-provider-layout title="Notifications">
    <x-panel title="Notification preferences">
        <form action="{{ route('provider.notifications.update') }}" method="POST" class="flex flex-col gap-3">
            @csrf @method('PATCH')
            <table class="w-full text-sm">
                <thead><tr class="text-[11px] uppercase text-textfaint text-left"><th class="pb-2 px-2">Event</th><th class="pb-2 px-2 text-right">Push</th><th class="pb-2 px-2 text-right">Email</th><th class="pb-2 px-2 text-right">WhatsApp</th></tr></thead>
                <tbody>
            ...
            <div class="flex justify-end"><button class="bg-ink text-white rounded-md px-3.5 py-2 text-xs font-semibold">Save preferences</button></div>
        </form>
    </x-panel>
</x-provider-layout>
