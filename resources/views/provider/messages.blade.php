    ...
    <div class="grid grid-cols-[260px_1fr] gap-3.5" x-data="{ active: 0 }">
        <x-panel title="Messages">
            <div class="flex flex-col gap-0.5">
                ...
                @empty
                    <div class="text-textfaint text-sm p-2">No conversations yet.</div>
                @endforelse
            </div>
        </x-panel>
        <x-panel title="Conversation">
            ...
            <div class="flex gap-2 mt-3.5">
                <input placeholder="Write a message…" class="flex-1 border border-line rounded-md px-2.5 py-2 text-sm">
                <button class="bg-ink text-white rounded-md px-3.5 py-2 text-xs font-semibold">Send</button>
            </div>
        </x-panel>
    </div>
</x-provider-layout>
