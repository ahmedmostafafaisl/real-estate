<x-site-layout :title="$serviceProvider->office_name.' — Keystone'">
    <div class="max-w-6xl mx-auto px-6 py-10">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <div class="text-sm text-textmute mt-1">{{ ucfirst($serviceProvider->provider_type) }} · {{ $serviceProvider->city->name ?? '' }} · Verified</div>
            </div>
        </div>
        @if ($serviceProvider->bio)<p class="text-textmute leading-relaxed mb-8 max-w-2xl">{{ $serviceProvider->bio }}</p>@endif

        <h2 class="font-serif text-xl mb-4">Active listings</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            @forelse ($properties as $p)
                <x-property-card :property="$p" />
            @empty
                <p class="text-textfaint col-span-3">No active listings right now.</p>
            @endforelse
        </div>
